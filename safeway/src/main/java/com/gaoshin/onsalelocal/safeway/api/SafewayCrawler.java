...
public class SafewayCrawler extends CrawlerBase {
	public List<WebCoupon> listWeekSpecial(String storeId) throws Exception {
		...
		String url = "http://mobile.safeway.com/WeeklySpecials?service=WeeklySpecials&storeId="
				+ storeId;
		Document doc = getDocumentFromUrl(url);
		List<Element> elements = selectElements(doc, "SPAN", "class",
				"items-list-span");
		for (Element ele : elements) {
			...
			Node catNode = ele.getParent().selectSingleNode(
					".//SPAN[@class='category-name-span']");
			String category = catNode.getText();
			String specialUrl = "http://mobile.safeway.com/WeeklySpecials/GetCategoryItems";
			String json = "{\"jsonItems\":\"" + eleTxt
					+ "\",\"service\":\"WeeklySpecials\",\"category\":\""
					+ category + "\"}";
			Document special = post(specialUrl,
					"application/json; charset=UTF-8", null, json);
			List<Element> products = selectElements(special, "A", "class",
					"black bold");
			for (Element prod : products) {
				...
				coupon.setUrl(link.href.startsWith("http") ? link.href
						: "http://m.safeway.com" + link.href);
			}
